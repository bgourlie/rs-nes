...
impl Cart for Nrom128 {
    fn new(rom: NesRom) -> Result<Self, &'static str> {
        if rom.prg.len() != PRG_BANK_SIZE * 3 {
            ...
            cart.prg_bank_switchable_1.copy_from_slice(&rom.prg[0..PRG_BANK_SIZE]);
            cart.prg_bank_switchable_2.copy_from_slice(&rom.prg[PRG_BANK_SIZE..(PRG_BANK_SIZE * 2)]);
            cart.prg_bank_fixed.copy_from_slice(&rom.prg[(PRG_BANK_SIZE * 2)..(PRG_BANK_SIZE * 3)]);
